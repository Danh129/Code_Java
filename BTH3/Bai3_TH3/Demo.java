...
/**
 *: Một công ty chuyên sản xuất sách và băng đĩa phục vụ học tập trong nhà trường 
phổ thông. Mỗi sản phẩm đều có mã sản phẩm, tên sản phẩm, mô tả sản phẩm, nhà sản 
xuất, giá bán. Ngoài ra:
o Sách có thêm thông tin số trang.
o Băng đĩa có thêm thông tin độ dài thời gian phát.
Xây dựng lớp cho phép quản lý danh sách sản phẩm tại công ty có các chức năng.
o Nhập thêm sản phẩm vào danh sách.
o Xóa sản phẩm khỏi danh sách.
o Xuất thông tin sản phẩm. Mỗi sản phẩm yêu cầu hiển thị đầy đủ thông tin từng thuộc 
tính của sản phẩm đó, mỗi thuộc tính hiển thị trên một dòng.
...
Viết phương thức main() chứa menu sau để người dùng chọn chức năng thực hiện.
1. Thêm sản phẩm.
2. Xoá sản phẩm, các phương thức cho phép truyền vào đối tượng sản phẩm hoặc 
chỉ là mã sản phẩm để xoá. 
3. Cập nhật sản phẩm tên sản phẩm, mô tả sản phẩm hoặc giá sản phẩm dựa trên 
mã sản phẩm. 
...
7. Thoát.
Trong đó, chức năng thêm sách người dùng sẽ nhập thông tin sách cần thêm, xoá sách 
người dùng nhập mã sản phẩm, tìm kiếm sách người dùng nhập từ khoá muốn tìm.
 * @author letha
 */
public class Demo {
    ...
    private static void timkiem(QuanLi ql){
        ...
        switch(op){
            ...
            case "4":{
                ...
                List<Product> date = ql.timtheophathanh(d);
                if(date.isEmpty()){
                    System.out.println("Khong hop le!");
                }
                else
